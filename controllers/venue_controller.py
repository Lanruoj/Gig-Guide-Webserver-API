...
# @venues.route("/watch/<int:venue_id>", methods=["DELETE"])
# @jwt_required()
# def delete_wv(venue_id):
#     # FETCH USER WITH user_id AS RETURNED BY get_jwt_identity() FROM JWT TOKEN
#     user = User.query.get(int(get_jwt_identity()))
#     if not user:
#         return abort(401, description="User must be logged in")
...
#     watched_venue = WatchVenue.query.filter(WatchVenue.venue_id==venue_id, WatchVenue.user_id==user.id).first()
